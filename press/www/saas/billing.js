...
function openSowaanCloudDashboard() {
	window.open(
		`${frappe_cloud_base_endpoint}/dashboard/sites/${frappe.boot.sitename}`,
		'_blank',
	);
}

function getTrialBannerDismissKey() {
	return `sowaan_trial_banner_dismissed_on:${frappe.boot.sitename}`;
}

function isTrialBannerDismissedToday() {
	return localStorage.getItem(getTrialBannerDismissKey()) === new Date().toDateString();
}
...
function generateTrialBanner() {
	...
	return $(`
		<style>
			...
			.trial-banner .trial-banner-close {
				display: flex;
				align-items: center;
				justify-content: center;
				width: 24px;
			...
			.trial-banner .trial-banner-close:hover {
		...
		<div class="trial-banner px-3 py-2 m-2 mt-4">
			<div>
				...
				<div class="text">
					<span class="title">Your trial ends in ${trial_end_string}.</span>
					<span class="description">${__('Please upgrade for uninterrupted services')}</span>
				</div>
			</div>
			<div class="actions">
				<button type="button" class="upgrade-plan-button px-2 py-1">
				...
				<button type="button" class="trial-banner-close" aria-label="${__('Dismiss')}">&times;</button>
			</div>
		</div>
	`);
}

function bindTrialBannerEvents($banner) {
	$banner.find('.upgrade-plan-button').on('click', openSowaanCloudDashboard);
	$banner.find('.trial-banner-close').on('click', function () {
		dismissTrialBannerForToday();
		$banner.remove();
	});
}
